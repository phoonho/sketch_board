...
@property (nonatomic, strong) LSDrawView *lsdDrawView;
...
- (void)configureView
{
    ...
    [self.drawImgView addSubview:self.lsdDrawView];
    [self.lsdDrawView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(self.drawImgView);
    }];
...
//    [self showColorFooterView];
    self.lsdDrawView.brushColor = UIColorFromRGB(112, 193, 80);
}

- (void)writeBtnDidSelected
{
    
    [self.lsdDrawView clean];
...
- (LSDrawView *)lsdDrawView
{
    if (!_lsdDrawView) {
        _lsdDrawView = [[LSDrawView alloc] init];
        _lsdDrawView.brushColor = UIColorFromRGB(255, 255, 255);
        _lsdDrawView.brushWidth = 6;
        _lsdDrawView.shapeType = LSShapeCurve;
    }
    return _lsdDrawView;
}
